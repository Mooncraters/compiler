...
bool checkToken(Type *now, const string& expectedToken, const string& functionName);
bool checkToken(Type *now, const string& expectedToken, const string& functionName){
    if(now->token != expectedToken){
        cout << functionName << " error" << endl;
        return false;
    }
    return true;
}
...
void getIdList(Type *now,vector< pair<string,int> >& res,bool reverseFlag){
    ...
    //     return;
    // }
    if (!checkToken(now, "idlist", "getIdList"))
        return;
...
void getArrayRangeList(Type *now,vector< pair<int,int> >& _arrayRangeList){
    ...
    //     return;
    // }
    if (!checkToken(now, "period", "getArrayRangeList"))
        return;
...
_Type* getType(Type *now){
    ...
    //     return NULL;
    // }
    if (!checkToken(now, "type", "getType"))
        return NULL;
...
void getVariant(Type *now,vector<_Variant*>& _variantList){
    ...
    //     return;
    // }
    if (!checkToken(now, "var_declaration", "getVariant"))
        return;
...
void getVariantList(Type *now,vector<_Variant*>& _variantList){
    ...
    //     return;
    // }
    if (!checkToken(now, "var_declarations", "getVariantList"))
        return;
...
void setConst(Type *now,_Constant* &_constant){//pascal在定义常量时，并没有指定常量的类型，所以需要自行判断
    ...
    //     return;
    // }
    if (!checkToken(now, "const_value", "setConst"))
        return;
    int loc=1;
...
void getConst(Type *now,vector<_Constant*>& _constantList){
    ...
    //     return;
    // }
    if (!checkToken(now, "const_declaration", "getConst"))
        return;
    int loc=int(now->children.size()-3);
...
void getConstList(Type *now,vector<_Constant*>& _constantList){
    ...
    //     return;
    // }
    if (!checkToken(now, "const_declarations", "getConstList"))
        return;
    if(now->children.size())
...
void getValueParameter(Type *now,vector<_FormalParameter*>& _formalParaList,int flag){
    ...
    //     return;
    // }
    if (!checkToken(now, "value_parameter", "getValueParameter"))
        return;
    vector< pair<string,int> > _idList;
...
void getParameter(Type *now,vector<_FormalParameter*>& _formalParaList){
    ...
    //     return;
    // }
    if (!checkToken(now, "parameter", "getParameter"))
        return;
...
void getFormalParameter(Type *now,vector<_FormalParameter*>& _formalParaList){
    ...
    //     return;
    // }
    if (!checkToken(now, "parameter_list", "getFormalParameter"))
        return;
    int loc=int(now->children.size()-1);
...
void getFormalParaList(Type *now,vector<_FormalParameter*>& _formalParaList){
    ...
    //     return;
    // }
    if (!checkToken(now, "formal_parameter", "getFormalParaList"))
        return;
    if(now->children.size())
...
void getSubprogramHead(Type *now,pair<string,int>& functionID,vector<_FormalParameter*>& _formalParaList,pair<string,int> &_type){
    ...
    //     return;
    // }
    if (!checkToken(now, "subprogram_head", "getSubprogramHead"))
        return;
...
_Expression* getTerm(Type *now){
	...
	// 	return NULL;
    // }
    if (!checkToken(now, "term", "getTerm"))
        return NULL;
...
_Expression* getSimpleExpression(Type *now){
	...
	// 	return NULL;
    // }
    if (!checkToken(now, "simple_expression", "getSimpleExpression"))
        return NULL;
    _Expression* _expression=NULL;
    if(now->children.size()==3){
        _expression = new _Expression;
...
_Expression* getExpression(Type *now){
	...
	// 	return NULL;
    // }
    if (!checkToken(now, "expression", "getExpression"))
        return NULL;
    _Expression* _expression=NULL;
    if(now->children.size()==3){
        _expression = new _Expression;
...
void getExpressionList(Type *now,vector<_Expression*>& _expressionList){
    ...
    //     return;
    // }
    if (!checkToken(now, "expression_list", "getExpressionList"))
        return;
    int loc=int(now->children.size()-1);
...
void getVariantReferenceList(Type *now,vector<_Expression*>& _expressionList){
    ...
    //     return;
    // }
    if (!checkToken(now, "id_varpart", "getVariantReferenceList"))
        return;
    if(now->children.size())
...
_VariantReference* getVariantReference(Type *now){
	...
	// 	return NULL;
    // }
    if (!checkToken(now, "variable", "getVariantReference"))
        return NULL;
...
_Statement* getElseStatement(Type *now){
    ...
    //     return NULL;
    // }
    if (!checkToken(now, "else_part", "getElseStatement"))
        return NULL;
...
_Statement* getProcedureCall(Type *now) {
	...
	// 	return NULL;
	// }
    if (!checkToken(now, "procedure_call", "getProcedureCall"))
		return NULL;
...
_Statement* getStatement(Type *now){
    ...
    //     return NULL;
    // }
    if (!checkToken(now, "statement", "getStatement"))
        return NULL;
	if (now->children.size() == 0)
		return NULL;
...
void getStatementList(Type *now,vector<_Statement*>& _statementList){
    ...
    //     return;
    // }
    if (!checkToken(now, "statement_list", "getStatementList"))
        return;
    int loc=int(now->children.size()-1);
...
_Compound* getCompoundStatement(Type *now){
    ...
    //     return NULL;
    // }
    if (!checkToken(now, "compound_statement", "getCompoundStatement"))
        return NULL;
...
void getSubprogramBody(Type *now,vector<_Constant*>& _constList,vector<_Variant*>& _variantList,_Compound* &_compound){
    ...
    //     return;
    // }
    if (!checkToken(now, "subprogram_body", "getSubprogramBody"))
        return;
...
_FunctionDefinition* getSubprogramDefinition(Type *now){
    ...
    //     return NULL;
    // }
    if (!checkToken(now, "subprogram", "getSubprogramDefinition"))
        return NULL;
...
void getSubprogramDefinitionList(Type *now,vector<_FunctionDefinition*>& _subprogramDefinitionList){
    ...
    //     return;
    // }
    if (!checkToken(now, "subprogram_declarations", "getSubprogramDefinitionList"))
        return;
	if (now->children.size()) {
...
_SubProgram* getProgramBody(Type *now){
    ...
    //     return NULL;
    // }
    if (!checkToken(now, "program_body", "getProgramBody"))
        return NULL;
...
void getProgramHead(Type *now,pair<string,int>& _programId,vector< pair<string,int> >& _paraList){
    ...
    //     return; 
    // }
    if (!checkToken(now, "program_head", "getProgramHead"))
        return;
...
_Program* getProgram(Type *now){
    ...
    //     return NULL;
    // }
    if (!checkToken(now, "programstruct", "getProgram"))
        return NULL;
